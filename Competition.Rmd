...
matplot(0:t, N$Species1, type = "p", pch =c(1,16), col = 1, ylim = c(0, 120), ylab="Number of individuals", xlab="Time")
...
In order to properly investigate the competition between two species, both need to have a non-zero density at *t=0*. Thus, in this tab you have to choose a starting vector lines this one, ```N0 <- c(10, 10)```, where both densities are different from zero.
...
In this part of the exercise, you investigate the effect of the various parameters on the *outcome of competition* between two species. Specifically, investigate the following questions.
  ...
  1) Let us focus on the case that both carrying capacities are equal, ```K1=K2```, and that the intrinsic growth rates are relatively small, say, ```r1 < 1``` and ```r2 < 1```. Under this scenario, look at the following cases:
$$
\begin{align}
